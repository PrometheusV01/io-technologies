...
export const getAuthors = (filter = '', page = 1, order = 'pageviews') => {
	...
	const filteredAuthors = authors
		.filter(({ name }) => !topAuthors.find((item) => item.name === name))
	...
	const authorsPerPage = [...topAuthors, ...filteredAuthors].slice(from, to);
        
	return {
		authors: authorsPerPage,
		page: page,
		total: Math.ceil(filteredAuthors.length / limit),
	};
};